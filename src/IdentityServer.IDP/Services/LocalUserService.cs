...
namespace IdentityServer.IDP.Services
{
    public class LocalUserService : ILocalUserService
    {
        ...
        public void AddUser(User userToAdd, string password)
        {
            ...
            if (_context.Users.Any(u => u.Phone == userToAdd.Phone))
            {
                throw new Exception("Phone must be unique");
            }

            userToAdd.EmailSecurityCode = Convert.ToBase64String(
                RandomNumberGenerator.GetBytes(128));
            userToAdd.EmailSecurityCodeExpirationDate = DateTime.UtcNow.AddHours(1);
            userToAdd.PhoneSecurityCode = Convert.ToBase64String(
                RandomNumberGenerator.GetBytes(128));
            userToAdd.PhoneSecurityCodeExpirationDate = DateTime.UtcNow.AddHours(1);
        ...
        public async Task<bool> ActivateUserAsync(string securityCode)
        {
            ...
            var user = await _context.Users.FirstOrDefaultAsync(u =>
                u.EmailSecurityCode == securityCode &&
                u.EmailSecurityCodeExpirationDate >= DateTime.UtcNow);
            ...
            user.Active = true;
            user.EmailSecurityCode = string.Empty;
            return true;
        }
